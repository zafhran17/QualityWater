...
void ph_func(){
   static unsigned long timepoint = millis();
    if(millis()-timepoint>1000U){                  //time interval: 1s
        timepoint = millis();
        vlt = analogRead(PH_PIN)/1024.0*5000;  // read the voltage
        phValue = ph.readPH(vlt,temperature);  // convert voltage to pH with temperature compensation
        //Serial.print("^C  pH:");
        phsen = phValue;
        Serial.println(phValue);
    }
    ph.calibration(vlt,temperature);           // calibration process by Serail CMD
    delay(20000);
}
**/
